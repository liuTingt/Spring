...
IOC（Inversion Of Controll）是一种设计思想，将原本在程序中创建对象的控制权交给spring框架来管理，IOC容器是spring 用来实现IOC的载体，IOC容器实际上就是一个Map(key,value)，map中存放的是各种对象。

**面向切面（AOP）**——允许通过分离应用的业务逻辑与系统级服务和事务进行内聚性的开发。可以不通过修改代码而改变逻辑。

能够将那些与业务逻辑无关，却为业务代码所共同调用的逻辑或责任（日志管理、权限控制等）封装起来，便于减少系统的重复代码，降低模块间的耦合度，有利于未来的可扩展性和维护性。
...
- Joinpoint(连接点):所谓连接点是指那些被拦截到的点，在spring中，这些点指的是方法，因为spring只支持方法类型的连接点。（类里面哪些方法可以被增强，这些方法成为连接点）
...
Joinpoint（连接点）：是程序执行的一个点。例如，一个方法的执行或者一个异常的处理。在Spring AOP中，一个连接点总是代表一个方法执行。

Pointcut（切点）：权限处理如权限校验、日志记录等在何处切入业务代码中（即织入切面）。切点分为execution方式和annotation方式。前者可以用路径表达式指定哪些类织入切面，后者可以指定被哪些注解修饰的代码织入切面。

Advice（通知）：包括通知时机和通知内容，通知内容就是要做什么事情，比如日志记录，通知时机就是在什么时机执行通知内容（前置通知、后置通知）

Aspect（切面）：即pointcut和Advice

Weaving（织入）：通过动态代理，在目标方法中执行通知内容的过程
...
spring AOP是基于动态代理的，如果要代理的对象实现了某个接口，那么spring AOP就会使用JDK动态代理创建代理对象，而没有实现接口的对象，转而使用cglib动态代理生成一个对代理队对象的子类来作为代理。
...
Spring AOP属于运行时增强，AspectJ AOP属于编译时增强。
Spring AOP基于代理（proxying），而AspectJ基于字节码操作。
...
注意：@Transactional事务无法捕捉被处理的异常（即方法中使用try catch处理异常，无法实现事务性）

## 11、Spring事务隔离

事务隔离级别：读未提交（脏读、不可重复读、幻读）、读提交（不可重复读、幻读）、重复读（幻读）、序列化（性能低）
